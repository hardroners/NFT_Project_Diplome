...
dataPDF = fs.readFileSync(diplome);
  
  
  const inputPDF = {
    apiKey,
    apiSecret,
    key: "/"+annee+"/"+filiere+"/"+nom+"_"+prenom+".pdf",
    data : dataPDF,
  };

  const resultPDF = await fleek.upload(inputPDF);
  ...
  const inputJSON = {
    apiKey,
    apiSecret,
    key: "/"+annee+"/"+filiere+"/"+nom+"_"+prenom+".json",
    data : dataJSON,
  };

  const resultJSON = await fleek.upload(inputJSON);
  ...
  const tokenContract = "0x762C32d22687A5e165840De55a8051bd9BC93666" // BAYC contract address
    ...
    const account = web3.eth.accounts.privateKeyToAccount("0x7a060745596d7a325327e6c726035414b81a4c59eaf4cdc051f45883c2e681f3")
    ...
    console.log("https://fleek.ipfs.io/ipfs/"+resultJSON.hash);
    contract.methods.create("https://fleek.ipfs.io/ipfs/"+resultJSON.hash).send({from: public_account, gas:1000000});
  
  
}

module.exports = { FunctionUpload }